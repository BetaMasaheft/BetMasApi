...
declare function dtsXML:citation($item as node()) {
	if ($item//t:titleStmt/t:title[@type = "short"]) then
		$item//t:titleStmt/t:title[@type = "short"]/text()
	else
		$item//t:titleStmt/t:title[@xml:id = "t1"]/text()
};
...
declare function dtsXML:get-workJSON($request as map(*)) {
	...
	return (
		...
		return if ($item//t:div[@type = "edition"]) then
			<json:value json:array="true">
				<id>{ data($recordid) }</id>
				<citation>{ dtsXML:citation($item) }</citation>
				<text>
					{
						let $t := string-join($item//t:div[@type = "edition"]//text(), " ")
					...
					<json:value json:array="true">
						{
							...
							return element {string($subtype/@subtype)} {
								($config:appUrl || "/api/dts/" || $id || "/" || $subtype/@n)
							}
						}
					</json:value>
				</contains>
	...
	return (
		...
		return if ($L1) then
			<json:value json:array="true">
				...
				<text>
					{
						let $t := string-join($L1//text(), " ")
						return normalize-space($t)
					}
				</text>
				{
					if (number($level1) > 1 and $item//t:div[@type = "edition"]/t:div[@n = (number($level1) - 1)]) then
						<previous>{ $config:appUrl }/api/dts/{ $id }/{ number($level1) - 1 }</previous>
					else (
					)
				}
				{
					if (number($level1) = count($item//t:div[@subtype = "book"])) then (
					) else if ($item//t:div[@type = "edition"]/t:div[@n = (number($level1) + 1)]) then
					...
					<json:value json:array="true">
						{
							...
							return element {string($subtype/@subtype)} {
								($config:appUrl || "/api/dts/" || $id || "/" || $level1 || "/" || $subtype/@n)
							}
						}
					</json:value>
				</contains>
		...
		return if (contains($line, "-")) then
			<json:value json:array="true">
				...
				</text>
				{
					if (number(substring-before($line, "-")) > 1) then
						<previous>
							{ $config:appUrl }/api/dts/{ $id }/{ $level1 }/{ number(substring-before($line, "-")) - 1 }
						</previous>
					else (
					)
				}
				{
					if (number($line) = count($L1//t:l[@n = substring-after($line, "-")])) then (
			...
			return if ($ltext) then
				...
				return <json:value json:array="true">
					...
					<title>
						{
							let $t := $item//t:titleStmt
							return $t/t:title[@xml:id = "t1"]/text()
						}
					</title>
					...
					{
						if (number($line) = count($ltext)) then (
		...
		return if (contains($line, "-")) then
			<json:value json:array="true">
				...
				<text>
					{
						for $l in (xs:integer(substring-before($line, "-")) to xs:integer(substring-after($line, "-")))
						return normalize-space($L2//t:l[@n = string($l)]//text())
					}
				</text>
				{
					if (number(substring-before($line, "-")) > 1) then
						<previous>
							{ $config:appUrl }/api/dts/{ $id }/{ $level1 }/{ $level2 }/{ number(substring-before($line, "-")) - 1 }
						</previous>
					else (
					)
				}
				{
					if (number($line) = count($L2//t:l[@n = substring-after($line, "-")])) then (
					) else if ($L2//t:l[@n = (number(substring-after($line, "-")) + 1)]) then
						<next>
							{ $config:appUrl }/api/dts/{ $id }/{ $level1 }/{ $level2 }/{ number(substring-after($line, "-")) + 1 }
						</next>
					else (
					)
				}
			...
			<json:value>
				{
					...
					return <json:value json:array="true">
						...
						{
							if (number($line) = count($L2//t:l[@n = $line])) then (
