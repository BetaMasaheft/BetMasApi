			...
			return (: we need a redirect in case the id contains already the passage. it should redirect the urn with passage to one which splits it and redirect it to a parametrized query :) if (
				...
				let $location := if ($parsedURN//s:group[@nr eq 15]/text() = "-") then (
					"permanent/" ||
						$sha ||
						"/api/dts/document?id=" ||
						$parsedURN//s:group[@nr eq 1]//text() ||
						$parsedURN//s:group[@nr eq 2]//text() ||
						$parsedURN//s:group[@nr eq 3]//text() ||
						"&amp;start=" ||
						$parsedURN//s:group[@nr eq 6]//text() ||
						"&amp;end=" ||
						$parsedURN//s:group[@nr eq 16]//text()
				) else (
					"permanent/" ||
						$sha ||
						"/api/dts/document?id=" ||
				...
				let $collection := if ($currentfile/@type eq "mss") then
					"Manuscripts"
				else if ($currentfile/@type eq "nar") then
					"Narrative"
				else
					"Works"
				let $permapath := replace(
					persdts:capitalize-first(substring-after(base-uri($cfile), "/db/apps/BetMasData/")),
					$collection,
					""
				)
				let $file := doc(
	...
	return if ($id = "") then (
		...
		let $witnesses :=
			...
			return if (starts-with($witness, "http")) then
				...
				let $mss := $persdts:collection-rootMS/id($witness)
		...
		let $passage := if ($file/@type eq "mss" and not($textType = "Inscription")) then (
			(: manuscripts :)
			...
			if ($ref != "") then
				let $l := if ($level = "") then
					...
					$level
				return dts:pasRef($l, $text, $ref, "unit", "mss", $manifest, $BMid)
			...
			(: no ref specified, list all main divs, assuming by the guidelines they are folios :)
				...
				dts:pasS($text/t:div[@n], "folio", "mss", $manifest, $BMid)
			(: if the level is not empty, than it has been specified to be either the second or third level, pages and columns :)
				...
				if ($level = "2") (: the pages of folios have been requested :) then
					dts:pasS($text//t:pb[@n], "page", "mss", $manifest, $BMid)
				else if ($level = "3") (: the columns of a pages have been requested :) then
					dts:pasS($text//(t:cb[@n]), "column", "mss", $manifest, $BMid)
				else if ($level = "4") (: the columns of a pages have been requested :) then
					dts:pasS($text//(t:lb[@n]), "line", "mss", $manifest, $BMid)
		...
		(: if a ref is specified show that navigation point :)
			...
			dts:pasRef(1, $text, $ref, "unit", "work", $witnesses, $BMid)
...
e.g. LIT1546Genesi&level=2
 :)
			...
			if ($ref != "") then
				dts:pasRef($level, $text, $ref, "unit", "work", $witnesses, $BMid)
			(: e.g. LIT1546Genesi&level=2 (max level is value of citeDepth!) :)
			else
				dts:pasLev($level, $text, "unit", "work", $witnesses, $BMid)
...
LIT1546Genesi&start=3&end=4 :) then
		...
		let $chunkedpassage := if (string($groupBy) != "") then (
			...
			let $refandtype := if ((count($p/type) eq 1) and ($p/type/text() != $ctype)) then
				...
				$refonly
			let $refTypeTitle := if (count($p/title) eq 1) then
				let $parttitle := map {"dc:title": $p/title/text()}
				return map:put($refandtype, "dts:dublincore", $parttitle)
			else
				$refandtype
			return $refTypeTitle
		...
		let $maximized := if (string($max) != "") then
			...
			$chunkedpassage
		let $cdepth := if (contains($id, "betmasMS:")) then
			3
		else (
			let $counts :=
				for $div in ($text//t:div[@type eq "textpart"], $text//t:l)
		...
		return if (count($text//t:ab//text()) le 1) then (
			...
			map {
				...
				"dts:passage":
					("permanent/" || $sha || "/" || "dts/api/document?id=" || $id || "{&amp;ref}{&amp;start}{&amp;end}"),
				"member": $maximized
			}
		)
};
