...
declare variable $jsonapi:response200Json := <rest:response>
	<http:response status="200">
		<http:header name="Content-Type" value="application/vnd.api+json; charset=utf-8" />
		<http:header name="Access-Control-Allow-Origin" value="*" />
	</http:response>
</rest:response>;

declare %rest:GET %rest:path("/json/api/{$id}") %output:method("json") function jsonapi:maintest($id as xs:string+) {
	...
	let $authors :=
		for $a in distinct-values($auths)
		return editors:editorKey($a)
	...
	let $langs :=
		for $language in $file//t:language/@ident
		return string($language)
	let $ptrs := ($file//t:*/@ref, $file//t:*/@passive)
	let $pointers :=
		for $p in distinct-values($ptrs)
		let $t := switch2:switchPrefix($p)
		return map {"type": $t, "id": string($p)}
	let $pointhere :=
		...
		return if ($MAINID = $id) then (
			...
			map {"type": switch2:switchPrefix($MAINID), "id": $MAINID}
	let $keywords :=
		for $k in $file//t:term/@key
		return map {"type": "auth", "id": string($k)}
	return (
		$jsonapi:response200Json,
		map {
			"jsonapi": map {"version": "1.0", "description": "test implementation of spec https://jsonapi.org/format/ "},
			"meta":
				map {
					...
					"sources":
						map {
							"TEI": "https://betamasaheft.eu/" || $id || ".xml",
			...
			"data":
				[
					map {
						"type": $type,
						"id": $id,
						"attributes": map {"title": $title, "languages": ($langs, "en")},
						"relationships":
							map {
								"pointers": map {"data": $pointers},
								"pointhere": map {"data": $pointhere},
								"keywords": map {"data": $keywords}
							}
					}
				]
		}
