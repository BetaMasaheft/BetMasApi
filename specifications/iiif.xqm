...
declare function iiif:manifestsource($item as node()) {
	(: ES :)
	if ($item//t:collection = "Ethio-SPaRe" or ($item//t:repository)[1]/@ref = "INS0339BML") then
		$config:appUrl || "/api/iiif/" || string($item/@xml:id) || "/manifest"
	else if ($item//t:collection = "EMIP") then
		$config:appUrl || "/api/iiif/" || string($item/@xml:id) || "/manifest"
	(: BNF :)
	else if ($item//t:repository[1][@ref eq "INS0303BNF"]) then
		replace($item//t:msIdentifier/t:idno/@facs, "ark:", "iiif/ark:") || "/manifest.json"
	(: vatican :)
	else if (starts-with($item//t:msIdentifier/t:idno/@facs, "http://digi.vatlib")) then
		replace($item//t:msIdentifier/t:idno/@facs, "http://", "https://")
	else
...
declare function iiif:locus($l as node()) {
	if ($l[@from][@to]) then (
		let $f := locus:folio($l/@from)
		let $t := locus:folio($l/@to)
		return if ($l/@facs and not($l/ancestor::t:TEI//t:repository/@ref eq "INS0303BNF")) then
			let $facsF := xs:integer(format-number(number($l/@facs), "###"))
			let $facsT := ($facsF + (xs:integer($t) - xs:integer($f)))
			for $x in $facsF to $facsT
			return $x
		else
			for $x in $f to $t
			return $x
		...
		if (contains($l/@target, " ")) then (
			if ($l/@facs and not($l/ancestor::t:TEI//t:repository/@ref eq "INS0303BNF")) then
				for $t in tokenize(normalize-space($l/@facs), " ")
				return format-number(number($t), "###")
			else
				for $t in tokenize(normalize-space($l/@target), " ")
				return locus:folio($t)
		) else (
			if ($l/@facs) then
				format-number(number($l/@facs), "###")
			else
				locus:folio($l/@target)
		)
	) else if ($l[@from][not(@to)]) then (
		if ($l/@facs and not($l/ancestor::t:TEI//t:repository/@ref = "INS0303BNF")) then
			format-number(number($l/@facs), "###")
		else
			locus:folio($l/@from)
	) else (
...
declare function iiif:range($iiifroot as xs:string, $structure as xs:string, $title as xs:string, $locusrange) {
	let $canvases :=
		for $folio in $locusrange
		return $iiifroot || "/canvas/p" || $folio
	let $cs := if (count($canvases) eq 1) then
		let $lastDigit :=
			let $thislast := substring-after($canvases, "/canvas/p")
			let $newnum := number($thislast) + 1
			return replace($canvases, "/p\d+$", ("/p" || xs:string($newnum)))
		return ($canvases, $lastDigit)
	else
		$canvases
	return map {
...
declare function iiif:rangetype(
	$iiifroot as xs:string,
	$name as xs:string,
	$title as xs:string,
	$seqran as xs:anyAtomicType+
) {
	map {
...
declare function iiif:ranges($iiifroot as xs:string, $ranges as node()) {
	...
	let $desc := if ($r/t:decoNote) then
		string-join(string:tei2string($r/t:decoNote/t:desc), " ") ||
			(
				if ($r/t/text()) then (
					" (" || $r/t || ")"
				) else (
	...
	else if ($r/t:item[starts-with(@xml:id, "a") or starts-with(@xml:id, "e")]) then
		string-join(string:tei2string($r/t:item/t:desc), " ") ||
			(
				if ($r/t/text()) then (
					" (" || $r/t || ")"
				) else (
	...
	else if ($r/t:item[starts-with(@xml:id, "q")]) then (
		if ($r/t/text()) then
			$r/t
		else (
		)
		...
		$r/t
	let $locusrange :=
		for $l in $r/t:*/t:locus
		return iiif:locus($l)

	return iiif:range($iiifroot, $r/r, $desc, $locusrange)
};

declare function iiif:rangeAndsubrange(
	$iiifroot as xs:string,
	$ranges as node(),
	$name as xs:string,
	$title as xs:string
	...
	let $seqran :=
		for $r in $ranges/range
		return $r/r
	return (iiif:rangetype($iiifroot, $name, $title, $seqran), iiif:ranges($iiifroot, $ranges))
};
...
declare function iiif:Canvases($item, $id, $iiifroot, $facsid) {
	...
	return for $graphic at $p in 1 to $tot
		let $n := $p
...
declare function iiif:msItemRange($msItem, $iiifroot) {
	<range>
		...
		<t>
			{
				if ($msItem/t:title[1]/text()) then
					string($msItem/t:title[1])
				else if ($msItem/t:title[1]/@ref) then
					exptit:printTitleID(substring-after($msItem/t:title[1]/@ref, "eu/"))
				else
					"item " || string($msItem/@xml:id)
			}
		</t>
		{ $msItem }
	</range>
...
declare function iiif:additionsRange($a, $iiifroot) {
	<range><r>{ $iiifroot || "/range/" || string($a/@xml:id) }</r><t>{ string($a/t:desc/@type) }</t>{ $a }</range>
};

declare function iiif:decorationsRange($d, $iiifroot) {
	<range><r>{ $iiifroot || "/range/" || string($d/@xml:id) }</r><t>{ string($d/@type) }</t>{ $d }</range>
};
	...
	let $ranges := <ranges>
		{
			...
			return switch ($elements)
				case "quires" return
					iiif:collationRange($n, $iiifroot)
				case "decorations" return
					iiif:decorationsRange($n, $iiifroot)
				case "additions" return
					iiif:additionsRange($n, $iiifroot)
				(: msitems :)
				default return
					iiif:msItemRange($n, $iiifroot)
		}
	</ranges>
	return iiif:rangeAndsubrange($iiifroot, $ranges, $elements, $rangetype)
};
...
declare function iiif:Structures($item, $iiifroot, $facsid) {
	...
	let $mainstructure := if ($items or $collation or $additions or $decorations) then (
		let $superRanges :=
			let $its := if ($items) then
				$iiifroot || "/range/" || "msItems"
			else (
			)
			let $colls := if ($collation) then
				$iiifroot || "/range/" || "quires"
			else (
			)
			let $adds := if ($additions) then
				$iiifroot || "/range/" || "additions"
			else (
			)
			let $decs := if ($decorations) then
				$iiifroot || "/range/" || "decorations"
			else (
			)
	...
	return ($mainstructure, $msItemStructures, $quiresStructures, $decorationStructures, $additionsStructures)
};
...
declare function iiif:multipleManifests($item as node()) {
	...
	return for $idno in $allIDNos
		...
		let $label := if ($parent/name() = "altIdentifier") then
			concat(exptit:printTitleID($item/@xml:id), ": subset for ", string($parent/t:idno))
		else
			exptit:printTitleID($item/@xml:id)
		let $facs := iiif:facsSwitch($idno)
		return if ($facs) then
			map {"@id": replace($facs, "ark:", "iiif/ark:") || "/manifest.json", "label": $label || $alt}
		else (
		)
};
...
declare function iiif:manifest($this as node()) {
	let $manifest := if (count($this//t:idno[@facs]) = 1) then
...
declare
	%rest:GET %rest:path("/api/iiif/collection/{$institutionid}") %output:method("json")
function iiif:RepoCollection($institutionid as xs:string) {
	(
		...
		let $mswithimages := if ($institutionid = "INS0447EMIP") then
			$repo[following-sibling::t:idno[@facs][@n]]
		else
			$repo[following-sibling::t:idno[@facs]]
		let $manifests :=
			...
			return map {"label": exptit:printTitleID($this/@xml:id), "@type": "sc:Manifest", "@id": $manifest}
...
declare %rest:GET %rest:path("/api/iiif/witnesses/{$workID}") %output:method("json") function iiif:WitnessesCollection(
	$workID as xs:string
) {
	(
		$iiif:response200,
		...
		let $listmanifests := (
			...
			return if ($ms//t:idno[@facs]) then
				let $manifest := iiif:manifestsource($ms)
				return map {"label": exptit:printTitleID($msid), "@type": "sc:Manifest", "@id": $manifest}
			else (
			),
			...
			return map {"label": $this, "@type": "sc:Manifest", "@id": $manifest}
		)
		let $manifests := if (count($listmanifests) eq 1) then
			[$listmanifests]
		else
			$listmanifests
		let $iiifroot := $config:appUrl || "/api/iiif/"
...
declare
	%rest:GET %rest:path("/api/iiif/{$id}/manifest") %rest:query-param("alt", "{$alt}", "") %output:method("json")
function iiif:manifest($id as xs:string*, $alt as xs:string*) {
	let $item := if (starts-with($id, "ES")) then
		collection($config:data-rootMS || "/ES")/id($id)
	else if (starts-with($id, "BML")) then
		collection($config:data-rootMS || "/FlorenceBML")/id($id)
	else if (starts-with($id, "EMIP")) then
		collection($config:data-rootMS || "/EMIP")/id($id)
	else
		$exptit:col/id($id)
	let $facsid := if ($alt = "") then
		($item//t:msIdentifier/t:idno[@facs], $item//t:place//t:desc[@facs])[1]
	else
		($item//t:altIdentifier[@xml:id eq $alt]/t:idno[@facs], $item//t:place//t:desc[@facs])[1]
	(: returns an attribute @facs :)
	let $facs := iiif:facsSwitch($facsid)
	return if ($facs) then (
		$iiif:response200,
		log:add-log-message("/api/iiif/" || $id || "/manifest", sm:id()//sm:real/sm:username/string(), "iiif"),
		let $institutionID := if ($item//t:repository) then
			string(($item//t:repository)[1]/@ref)
		else
			$id
		...
		let $url := $config:appUrl ||
			(
				if ($item/ancestor::t:TEI/@type = "ins") then
					"/institutions/"
				else
					"/manuscripts/"
			) ||
		...
		let $thumbid := $imagesbaseurl ||
			...
			"001.tif/full/80,100/0/default.jpg"
		...
		(: this is where the manifest is :)
		let $request := $iiifroot ||
			"/manifest" ||
			(
				if ($facsid/parent::t:altIdentifier/@xml:id) then
					"?alt=" || string($facsid/parent::t:altIdentifier/@xml:id)
				else if ($facsid/parent::t:altIdentifier) then
					"?alt=alt"
				else (
				)
			)
		(: this is where the sequence is :)
		let $attribution := if ($item//t:repository[1][@ref eq "INS0339BML"]) then (
			"The images of the manuscript taken by Antonella Brita, Karsten Helmholz and Susanne Hummel during a mission funded by the Sonderforschungsbereich 950 Manuskriptkulturen in Asien, Afrika und Europa, the ERC Advanced Grant TraCES, From Translation to Creation: Changes in Ethiopic Style and Lexicon from Late Antiquity to the Middle Ages (Grant Agreement no. 338756) and Beta maṣāḥǝft. The images are published in conjunction with this descriptive data about the manuscript with the permission of the https://www.bmlonline.it/la-biblioteca/cataloghi/, prot. 190/28.13.10.01/2.23 of the 24 January 2019 and are available for research purposes."
		) else
			"Provided by " ||
				(
					if ($item//t:collection/text()) then
						string-join($item//t:collection/text(), ", ")
					else
						"Bm"
				) ||
				" project. " ||
				(
					if ($item//t:editionStmt/t:p) then
						string-join($item//t:editionStmt/t:p/string())
					else
						" "
				)
		let $logo := if ($item//t:repository[1][@ref eq "INS0339BML"]) then (
			"/rest/BetMasWeb/resources/images/logobml.png"
		) else
			"/rest/BetMasWeb/resources/images/logo" ||
				string-join($item//t:collection[1][not(matches(., "\s"))]/text()) ||
				".png"
		let $sequence := $iiifroot || "/sequence/normal"
		let $parent := $facsid/parent::node()
		let $label := if ($parent/name() = "altIdentifier") then
			concat(exptit:printTitleID($item/@xml:id), ": subset for ", string($parent/@xml:id))
		else
			exptit:printTitleID($item/@xml:id)
		...
		return map {
			...
			"metadata":
				[
					...
					map {"label": "object type", "value": [map {"@value": $objectType, "@language": "en"}]},
					map {"label": "main view", "value": $config:appUrl || "/" || $id}
				],
			...
			"logo": map {"@id": $config:appUrl || $logo},
			"rendering": map {"@id": $url, "label": "web presentation", "format": "text/html"},
		...
		$iiif:response400, map {"info": ("no manifest available for " || $id)}
	)
};

declare function iiif:rangeexistence($nodes, $iiifroot, $element, $name) {
	if ($nodes) then
		iiif:makeRanges($nodes, $iiifroot, $element, $name)
	else (
	)
};

declare
	%rest:GET %rest:path("/api/iiif/{$id}/range/{$rangeId}") %rest:query-param("alt", "{$alt}", "") %output:method("json")
function iiif:singerange($id as xs:string*, $rangeId as xs:string*, $alt as xs:string*) {
	...
	return if ($facs) then (
		...
		return if ($rangeId = $mainranges) then (
			(: a superrange :)
			switch ($rangeId)
				case "decorations" return
					iiif:rangeexistence($item//t:decoNote[.//t:locus], $iiifroot, "decorations", "Decorations")
				case "additions" return
					iiif:rangeexistence(
						$item//t:additions/t:list/t:item[.//t:locus],
						$iiifroot,
						"additions",
						"Additions and Extras"
					)
				case "quires" return
					iiif:rangeexistence($item//t:collation/t:list/t:item[.//t:locus], $iiifroot, "quires", "Collation")
				(: default on msItems :)
				default return
					iiif:rangeexistence($item//t:msItem[t:locus][t:title[@ref]], $iiifroot, "msItems", "Contents")
		) else if (count($item/id($rangeId)) = 1) then (
			let $singleRange := $item/id($rangeId)
			let $elements := switch ($rangeId)
				case starts-with($rangeId, "a") return
					"additions"
				case starts-with($rangeId, "e") return
					"additions"
				case starts-with($rangeId, "d") return
					"decorations"
				case starts-with($rangeId, "q") return
					"quires"
				default return
					"msItems"
			...
			let $rangeType := switch ($rangeId)
				...
				default return
					"Contents"
			return iiif:makeRanges($singleRange, $iiifroot, $elements, $rangeType)
		) else (
			$iiif:response400, map {"info": ("no rage available for " || $rangeId || " in " || $id)}
		)
	) else (
		$iiif:response400, map {"info": ("no manifest available for " || $id)}
	)
...
declare
	%rest:GET %rest:path("/api/iiif/{$id}/sequence/normal") %rest:query-param("alt", "{$alt}", "") %output:method("json")
...
declare
	%rest:GET %rest:path("/api/iiif/{$id}/canvas/p{$n}") %rest:query-param("alt", "{$alt}", "") %output:method("json")
