...
declare function persiiif:capitalize-first($arg as xs:string?) as xs:string? {
	concat(upper-case(substring($arg, 1, 1)), substring($arg, 2))
};

declare function persiiif:fileingit($bmID, $sha) {
	let $permapath := replace(
		persiiif:capitalize-first(
			substring-after(base-uri($exptit:col/id($bmID)[name() eq "TEI"]), "/db/apps/BetMasData/")
		),
		"Manuscripts",
		""
	)
	return doc("https://raw.githubusercontent.com/BetaMasaheft/Manuscripts/" || $sha || "/" || $permapath)//t:TEI
};
...
declare
	%rest:GET %rest:path("/permanent/{$sha}/api/iiif/{$id}/manifest") %output:method("json")
function persiiif:manifest($id as xs:string*, $sha as xs:string*) {
	let $item := persiiif:fileingit($id, $sha)
	return if ($item//t:msIdentifier/t:idno/@facs) then (
		$iiif:response200,
		...
		let $thumbid := $imagesbaseurl ||
			(
				if ($item//t:collection = "EMIP") then (
		...
		let $attribution := if ($item//t:repository/@ref eq "INS0339BML") then (
			"The images of the manuscript taken by Antonella Brita, Karsten Helmholz and Susanne Hummel during a mission funded by the Sonderforschungsbereich 950 Manuskriptkulturen in Asien, Afrika und Europa, the ERC Advanced Grant TraCES, From Translation to Creation: Changes in Ethiopic Style and Lexicon from Late Antiquity to the Middle Ages (Grant Agreement no. 338756) and Beta maṣāḥǝft. The images are published in conjunction with this descriptive data about the manuscript with the permission of the https://www.bmlonline.it/la-biblioteca/cataloghi/, prot. 190/28.13.10.01/2.23 of the 24 January 2019 and are available for research purposes."
		) else
			"Provided by " || $item//t:collection/text() || " project."
		let $logo := if ($item//t:repository/@ref eq "INS0339BML") then (
			"/rest/BetMasWeb/resources/images/logobml.png"
		) else
			"/rest/BetMasWeb/resources/images/logo" || $item//t:collection/text() || ".png"
...
declare
	%rest:GET %rest:path("/permanent/{$sha}/api/iiif/{$id}/sequence/normal") %output:method("json")
...
declare
	%rest:GET %rest:path("/permanent/{$sha}/api/iiif/{$id}/canvas/p{$n}") %output:method("json")
