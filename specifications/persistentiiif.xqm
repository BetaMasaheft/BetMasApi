...
declare function persiiif:manifest($request as map(*)) {
	let $id as xs:string* := $request?parameters?id
	let $sha as xs:string* := $request?parameters?sha
	return let $item := persiiif:fileingit($id, $sha)
...
declare function persiiif:sequence($request as map(*)) {
	let $id as xs:string* := $request?parameters?id
	let $sha as xs:string* := $request?parameters?sha
	return (
...
declare function persiiif:canvas($request as map(*)) {
	let $id as xs:string* := $request?parameters?id
