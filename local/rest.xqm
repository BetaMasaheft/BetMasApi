...
declare %rest:GET %rest:path("/api/listRepositoriesName") %output:method("html") function api:listRepositoriesName() {
...
declare %rest:GET %rest:path("/api/cataloguesZotero") %output:method("html") function api:getcataloguesZotero() {
...
declare %rest:GET %rest:path("/api/latest") %output:method("json") function api:latest() {
	(
		$api:response200Json,
...
(:~
 ...
 :)
declare
	%rest:GET %rest:path("/api/string/{$id}") %rest:query-param("element", "{$element}", "") %output:method("text")
...
declare
	...
	%rest:query-param("element", "{$element}", "")
	%output:method("xml")
	%test:args("IVefiopsk1", "a1", "item")
	%test:assertXPath("//element")
...
function api:get-POSTPROCESSED-tei-by-ID($id as xs:string) {
	...
	return ($api:response200XML, $doc)
};

(:~
 : given the file id, returns the source TEI in a json serialization
 :)
declare %rest:GET %rest:path("/api/{$id}/json") %output:method("json") function api:get-tei2json-by-ID(
	$id as xs:string
) {
	let $log := log:add-log-message("/api/" || $id || "/json", sm:id()//sm:real/sm:username/string(), "REST")
	let $login := xmldb:login($config:data-root, $config:ADMIN, $config:ppw)
	return ($api:response200Json, <json:value>{ api:get-tei-rec-by-ID($id) }</json:value>)
};
