...
declare %test:args("PRS8249Ruppell", "person") %test:assertExists function att:attestations($request as map(*)) {
	...
	return let $ty := switch ($type)
		...
		let $hits :=
			...
			return if ($MAINID = $id) then (
			) else
				let $titleRoot := exptit:printTitleID($MAINID)
				let $atts :=
					for $a at $p in $att
					let $element := $a/name()
					let $text := if ($a/text()) then
						$a/text()
					else if ($a/t:label) then
						string:tei2string($a/t:label)
					else
						"pointer only"
					...
					let $date := if ($a/ancestor::t:item[1]//t:date) then
						att:stringDates($a/ancestor::t:item[1]//t:date)
					else if ($a/ancestor::t:msItem//t:date) then
						att:stringDates($a/ancestor::t:msItem[1]//t:date)
					else if ($a/ancestor::t:handNote//t:date) then
						att:stringDates($a/ancestor::t:handNote[1]//t:date)
					else if ($a/ancestor::t:decoNote//t:date) then
						att:stringDates($a/ancestor::t:decoNote[1]//t:date)
					else
						"no date"
					let $MainRole := switch ($element)
						case "persName" return
							string($a/@role)
						case "div" return
							string($a/@type) || " - " || string($a/@subtype)
					...
					let $occpers := if (count($cooccurringPers) gt 0) then (
						let $persons :=
							for $pers in $cooccurringPers
							let $id := if ($pers/@ref) then
								string($pers/@ref)
							else
								"no-id"
							let $name := if ($pers/text()) then
								$pers/text()
							else
								exptit:printTitleID($pers/@ref)
							let $thisrole := if ($pers/@role) then
								string($pers/@role)
							else (
							)
							return map {"id": $id, "name": $name, "type": $thisrole}
						return map {"type": "persons", "persons": $persons}
					) else (
					)
					let $occplace := if (count($cooccurringPlace) gt 0) then (
						let $places :=
							for $place in $cooccurringPlace
							let $id := if ($place/@ref) then
								string($place/@ref)
							else
								"no-id"
							let $name := if ($place/text()) then
								$place/text()
							else
								exptit:printTitleID($place/@ref)
							let $thistype := if ($place/@type) then
								string($place/@type)
							else (
							)
							return map {"id": $id, "name": $name, "type": $thistype}
						return map {"type": "places", "places": $places}
					) else (
					)
					let $occwork := if (count($cooccurringworks) gt 0) then (
						let $works :=
							for $work in $cooccurringworks
							let $id := if ($work/@ref) then
								string($work/@ref)
							else
								"no-id"
							let $name := if ($work/text()) then
								$work/text()
							else
								exptit:printTitleID($work/@ref)

							return map {"id": $id, "name": $name}
						return map {"type": "works", "works": $works}
					) else (
					)
					let $occterm := if (count($cooccurringterm) gt 0) then (
						let $terms :=
							for $term in $cooccurringterm
							let $id := if ($term/@key) then
								string($term/@key)
							else
								"no-id"
							let $name := if ($term/text() and $term/@key) then
								$term/text()
							else if (not($term/@key)) then
								$term/text()
							else
								exptit:printTitleID($term/@key)

							return map {"id": $id, "name": $name}
						return map {"type": "terms", "terms": $terms}
					) else (
					)
