...
(:~
 ...
 :)
declare
	%rest:GET %rest:path("/api/RoleAttestations") %rest:query-param("role", "{$role}", "") %output:method("html")
...
function roles:role($role as xs:string*) {
	...
	return ($config:response200Json, map {"role": $role, "hits": $hits, "total": count($hits), "referring": $total})
};
...
declare
	...
	%output:method("json")
	%test:args("patron", "PRS2916Bruce")
	%test:assertExists
	%test:args("owner", "PRS2916Bruce")
	%test:assertExists
function roles:roleID($role as xs:string*, $ID as xs:string*) {
	let $ID := if (starts-with($ID, "https://betamasaheft.eu/")) then
		string($ID)
	else
	...
	return (
		...
		map {"pwl": $ID, "title": exptit:printTitleID($ID), "hits": count($path), "hasthisrole": $hs}
	)
};
