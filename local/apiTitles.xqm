...
(:~
 ...
 :)
declare %rest:GET %rest:path("/api/{$id}/title") %output:method("text") function apiTit:get-FormattedTitle(
	$id as xs:string
) {
	(
		$config:response200,
...
(:~
 : given the file id, returns the main title
 :)
declare %rest:GET %rest:path("/api/{$id}/title/json") %output:method("json") function apiTit:get-FormattedTitleJson(
	$id as xs:string
) {
	(
...
(:~
 ...
 :)
declare
	%rest:GET %rest:path("/api/{$id}/{$SUBid}/title") %output:method("text")
