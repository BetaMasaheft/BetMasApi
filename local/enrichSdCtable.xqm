...
declare function enrich:teipart($request as map(*)) {
	...
	return (
		...
		let $name := $node/name()
		let $NodeMap := map {}
		let $NodeMapwithxmllang := if ($node/@xml:lang) then
			map:put($NodeMap, "xml:lang", string($node/@xml:lang))
		else
			$NodeMap
		let $NodeMapwithtitle := if ($node/t:title) then
			let $titString := if ($node/t:title/@ref) then
				let $titRef := string($node/t:title/@ref)
				return exptit:printTitleID($titRef)
			else
				$node/t:title/text()
		...
		let $NodeMapwithbibl := if ($node//t:bibl) then
			let $bibstrings :=
				for $b in $node//t:bibl
				return string:tei2string($b)
		...
		let $NodeMapwithdesctype := if ($node/t:desc/@type) then
			let $desctypeString := exptit:printTitleID(string($node/t:desc/@type))
		...
		let $NodeMapwithlinks := if ($node//t:*[@ref] or $node//t:term or $node//t:ref) then
			let $named :=
				for $ne in $node//t:*[@ref]
				return exptit:printTitleID($ne/@ref) ||
					(
						if ($ne/@role) then
							" (" || string($ne/@role) || ")"
						else (
			...
			let $terms :=
				for $t in $node//t:term/@key
				return exptit:printTitleID($t)
			let $refs :=
				for $r in $node//t:ref
				return string:tei2string($r)
			let $all := (
				"Named Entities: " ||
					string-join($named, ", ") ||
					" | Terms: " ||
