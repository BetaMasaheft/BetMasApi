...
declare
%rest:GET
%rest:path("/api/enrichMe/{$id}/{$anchor}")
%output:method("json")
function enrich:teipart($id as xs:string, $anchor as xs:string*){
 ...
 let $NodeMapwithlinks := if($node//t:*[@ref ] or $node//t:term or $node//t:ref) then 
                              let $named := for $ne in $node//t:*[@ref] return exptit:printTitleID($ne/@ref) || (if($ne/@role)then ' ('||string($ne/@role)||')' else ())
                              let $terms := for $t in $node//t:term/@key return exptit:printTitleID($t)
