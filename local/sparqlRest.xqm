...
declare %rest:GET %rest:path("/BetMasWeb/{$id}/rdf") function apisparql:constructURIid($id as xs:string*) {
	(
...
declare
	%rest:GET %rest:path("/BetMasWeb/{$id}/{$class}/{$subid}/rdf") %output:method("xml")
...
WHERE {{<" ||
	...
	let $xml := fusekisparql:query("betamasaheft", $q)
	return ($apisparql:response200XML, $xml)
};
...
https://betamasaheft.eu/BNFet32/place/annotation/1
 :)
declare
	%rest:GET %rest:path("/BetMasWeb/{$id}/{$class}/annotation/{$n}/rdf") %output:method("xml")
...
declare %rest:GET %rest:path("/BetMasWeb/bond/{$bond}/rdf") %output:method("xml") function apisparql:constructURIbond(
	$bond as xs:string*
) {
...
declare
	%rest:GET %rest:path("/api/SPARQL") %rest:query-param("query", "{$query}", "") %output:method("xml")
...
declare
	%rest:GET %rest:path("/api/RDFXML/{$collection}/{$id}") %output:method("xml")
function apisparql:sparqlITEMgraph($collection as xs:string, $id as xs:string) {
	let $xml := doc("/db/rdf/" || $collection || "/" || $id || ".rdf")//rdf:RDF
	return ($apisparql:response200RDFXML, $xml)
};

declare
	%rest:GET %rest:path("/api/RDFJSON/{$collection}/{$id}") %output:method("json")
function apisparql:sparqlITEMgraphJson($collection as xs:string, $id as xs:string) {
	...
	return ($apisparql:response200RDFJSON, $json)
};

declare
	%rest:GET %rest:path("/api/SPARQL/json") %rest:query-param("query", "{$query}", "") %output:method("json")
function apisparql:sparqljsonQuery($query as xs:string*) {
	...
	return ($apisparql:response200Json, $json)
};
...
declare %rest:GET %rest:path("/api/SPARQL/relations/{$id}") %output:method("json") function apisparql:sparqlQueryJSON(
	$id as xs:string*
) {
	let $query := (
		$apisparql:prefixes || "
...
?x crm:P48_has_preferred_identifier ?y}"
	...
	return if (count($results) >= 1) then (
		...
		$apisparql:response200Json, map {"info": "sorry, no relations available in our RDF data"}
	)
};

declare %rest:GET %rest:path("/api/SPARQL/SdCunits/{$type}") %output:method("json") function apisparql:SdCunits(
	$type as xs:string*
) {
...
declare
	%rest:GET %rest:path("/api/SPARQL/versions/{$id}/{$chapterID}") %output:method("json")
