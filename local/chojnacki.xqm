...
(:~
 ...
 :)
declare function chojnacki:allChojnacki($request as map(*)) {
	let $id as xs:string* := $request?parameters?id
	return let $Chojnacki := collection($config:data-rootCh)//marc:record[descendant::marc:subfield[@code = "a"][. = $id]]
		let $ChojnackItems :=
			for $Choj in $Chojnacki
			...
			return map {"name": $name, "link": $link, "digvatID": $DigiVatID, "segnatura": $DigiVatSegnatura}
		return if (count($ChojnackItems) ge 1) then
			map {"total": count($ChojnackItems), "ChojnackItems": $ChojnackItems}
		else if (count($ChojnackItems) eq 1) then
			map {"total": 1, "ChojnackItems": [$ChojnackItems]}
		else
			map {"total": 0, "info": "sorry, there are no related items in the Chojnacki Collection at the Vatic Library."}
};
