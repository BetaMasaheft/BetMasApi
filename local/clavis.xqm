...
(:~
 ...
 :)
declare %rest:GET %rest:path("/api/clavis/{$id}") %output:method("json") function clavis:ClavisbyID($id as xs:string*) {
	(: results from PAThs
...
se interrogato da titolo da gli id, altrimenti per id da gli altri id e il titolo
 ...
 :)
	...
	return ($config:response200Json, map {"CAe": $id, "title": $title, "clavis": $clavisIDS})
};
...
(:~
 ...
 :)
declare
	%rest:GET %rest:path("/api/clavis/all") %rest:query-param("type", "{$type}", "") %output:method("json")
...
se interrogato da titolo da gli id, altrimenti per id da gli altri id e il titolo
 ...
 :)
	...
	return ($config:response200Json, map {"results": $results, "total": count($results)})
};
...
se interrogato da titolo da gli id, altrimenti per id da gli altri id e il titolo
 ...
 :)
declare
	%rest:GET %rest:path("/api/clavis") %rest:query-param("q", "{$q}", "") %output:method("json")
function clavis:Clavis($q as xs:string*) {
	let $eval-string := concat("collection($config:data-rootW)//t:TEI", "[ft:query(.,'", $q, "')]")
	...
	return if (count($hits) gt 0) then (
		$config:response200Json, map {"items": $hi, "totalhits": $c}
	) else (
