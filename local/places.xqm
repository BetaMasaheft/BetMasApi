...
declare variable $places:placeprefixes :=
	"
@prefix cnt: &lt;http://www.w3.org/2011/content#&gt; .
@prefix dcterms: &lt;http://purl.org/dc/terms/&gt; .
@prefix foaf: &lt;http://xmlns.com/foaf/0.1/&gt; .
...
declare function places:JSONfile($item as node(), $id as xs:string) {
	let $regions := if ($item//t:region[@ref]) then
		for $region in $item//t:region[@ref]
		return ann:getannotationbody($region/@ref)
	else (
	)
	let $settlements := if ($item//t:settlement[@ref]) then
		for $settl in $item//t:settlement[@ref]
		return ann:getannotationbody($settl/@ref)
	else (
	)
	let $connects := ($regions, $settlements, "https://pleiades.stoa.org/places/39274")
	let $creators :=
		for $c in config:distinct-values($item//t:revisionDesc/t:change[contains(., "created")]/@who)
		return map {"name": editors:editorKey($c)}
	let $contributors :=
		for $c in config:distinct-values($item//t:revisionDesc/t:change/@who)
		return map {"name": editors:editorKey($c)}
	let $periods := if ($item//t:state) then
		for $c in $item//t:state[@type eq "existence"]/@ref
		return exptit:printTitleID($c)
	else (
	)
	let $names :=
		for $name in $item//t:place/t:placeName
		let $nID := $name/@xml:id
		return map {
			"association_certainty": "certain",
			"attested": normalize-space($name/text()[1]),
			"romanized":
				for $corr in $item//t:placeName[contains(@corresp, $nID)]
				return normalize-space($corr/text()[1]),
			"transcription_accuracy": "accurate",
			"transcription_completeness": "complete",
			"uri":
				if ($name/@xml:id) then (
					$places:bmurl || "/" || $id || "#" || $name/@xml:id
				) else (
				)
		}
	let $pt := $item//t:place/@type
	let $types := if (contains($pt, " ")) then
		for $t in tokenize($pt, " ")
		return normalize-space($t)
	else
		string($pt)
	let $bibls :=
		for $b in $item//t:bibl
		return map {
	...
	let $coords := if ($item//t:geo[@rend eq "polygon"]) then (
	...
	return map {
		"@context":
			map {
				...
				"bbox": map {"@container": "@list", "@id": "geojson:bbox"},
				"connectsWith": "_:n8",
				"coordinates": "geojson:coordinates",
				"description": "http://purl.org/dc/terms/description",
				"features": map {"@container": "@set", "@id": "geojson:features"},
		...
		"bbox":
			if (count($coords) gt 2) then
				for $firstandsecond in subsequence($coords, 1, 2)
				return (
					for $c in tokenize($firstandsecond, ",")
					return number($c)
				)
			else (
				$coords, $coords
		...
		"citation":
			"Location based on Encyclopaedia Aethiopica, from the Beta maṣāḥǝft: Manuscripts of Ethiopia and Eritrea project",
		"connectsWith": [$connects],
		"creators": $creators,
		"contributors": $contributors,
		"description":
			if ($item//t:desc[@type eq "foundation"]) then
				normalize-space(string-join(string:tei2string($item//t:desc[@type eq "foundation"]), ""))
			else (
			),
		"details":
			if ($item//t:ab[@type eq "history"]) then
				normalize-space(string-join(string:tei2string($item//t:ab[@type eq "history"]), ""))
			else (
			),
		"geometry":
			map {
								...
								array
									{
										...
										return let $array := array
												{
													for $c in tokenize($latlng, ",")
													return number($c)
												}
											return $array
									}
		...
		"properties":
			map {
				...
				"snippet":
					if ($item//t:ab[@type eq "foundation"]) then
						normalize-space(string-join(string:tei2string($item//t:ab[@type eq "foundation"]), ""))
					else (
					),
				"title": $title
			},
		"type": "Feature",
		"history": [map {"comment": "Mapped to Json from TEI xml", "modified": current-dateTime(), "principal": "pliuzzo"}],
		"names": $names,
...
declare %rest:GET %rest:path("/api/KML/place/{$placeid}") %output:method("xml") function places:kmlPlaceAttestation(
	...
	if (
		...
		return <kml>
			{
				for $place in $items
				return places:placeMark($place)
			}
		</kml>
	) else
		<info>Sorry! {
				$placeid
			} is not a valid id! We use our own place ids, which start with LOC or INS, Q items in Wikidata like Q115, Ethiopia or geonames id in the format gn:000000.</info>
};
		...
		<Point>
			{
				...
				return <coordinates>
					{
						if (matches($coordinates, "\d+\.?\d*,\d+\.?\d*")) then
							$coordinates
						else (
						)
					}
...
declare function places:decidePlaceNameSource($pRef as xs:string) {
	if ($exptit:placeNamesList//t:item[@corresp = $pRef]) then
		...
		return $exptit:col/id($onlyId)//t:title[@type = "full"]/text()
};
...
declare function places:placeMark($place as node()) {
	...
	return (: if($pRec//t:coord) then :) <Placemark>
		...
		<Point><coordinates>{ coord:invertCoord(coord:getCoords($pId)) }</coordinates></Point>
		<TimeStamp>
			<when>
				{
					let $dates := ($place/@when, $place/@notBefore, $place/@notAfter)
					return max($dates)
				}
			</when>
		</TimeStamp>
...
declare function places:datePlaceMark($datePlace as node()) {
	let $corresp := if (contains($datePlace/@corresp, " ")) then
		for $c in tokenize($datePlace/@corresp, " ")
		return string($c)
	else
		$datePlace/@corresp
	...
	return (: if($pRec//t:coord) then :) <Placemark>
		...
		<description>
			{ $datePlace/name() } of { string($root//t:TEI/@xml:id) }
			{
				if ($corresp[starts-with(., "#t")]) then
					$corresp[starts-with(., "#t")]
				else (
				)
			}
		</description>
		<name>{ $pRef }</name>
		<Point><coordinates>{ coord:getCoords($pRef) }</coordinates></Point>
		{
			if ($datePlace/@when) then (
				<TimeStamp><when>{ $datePlace/@when }</when></TimeStamp>
			) else
				<TimeSpan><being>{ $datePlace/@notBefore }</being><end>{ $datePlace/@notAfter }</end></TimeSpan>
		}
	</Placemark>
(: handling of when is at the moment nonsensical, as it just takes a maximum. needs to check quality and format the date correctly
see geobrowser data specification :)
...
declare function places:annotation($this, $r, $x, $mode) {
 ...
 &lt;" ||
				...
				"&gt; ;
                oa:hasBody &lt;" ||
				ann:getannotationbody(string($this/@ref)) ||
				'&gt; ;
                oa:annotatedAt "' ||
				current-dateTime() ||
				'"^^xsd:date ;
...
declare function places:ThisAnnotatedThing($r, $tit, $mode as xs:string) {
 ...
 dcterms:description "' ||
		(
			if ($mode = "works") then (
				"A literary work in the Ethiopian tradition (CAe " || substring($r, 4, 4) || ")."
			) else if ($mode = "manuscripts") then (
				"A manuscript part of the Ethiopian literary tradition (" || string($r) || ")."
			) else (
			)
		) ||
		'";
...
declare %rest:GET %rest:path("/api/placeNames/void") %output:method("text") function places:placesInWorksTTLVoid() {
	...
	let $annotationsMS :=
		for $d in $dataMS
		group by $r := root($d)//t:TEI/@xml:id
		return " void:dataDump <https://betamasaheft.eu/api/placeNames/manuscripts/" || string($r) || ">"
	let $annotationsW :=
		for $d in $dataW
		group by $r := root($d)//t:TEI/@xml:id
		return " void:dataDump <https://betamasaheft.eu/api/placeNames/works/" || string($r) || ">"
...
declare function places:pelagiosDump() {
	...
	let $filecontent :=
		let $annotations :=
			...
			let $annotatedthing := if ($tit) then
				try { ann:annotatedThing($d, $tit[1], $i) } catch * { ($i || $err:description) }
			else (
			)
			order by $tit[1]
	...
	return "stored " || $txtarchive || $filename
};
